...
async function buscarPessoa() {
    ...
    searchId.focus();
    try {
        const response = await fetch(`${API_BASE_URL}/pessoa/api/${id}`);
...
async function salvarOperacao() {
    ...
    // Determinar tipo_usuario baseado nos checkboxes
    let tipo_usuario = 'C'; // Padrão: Cliente
    if (document.getElementById('checkboxFuncionario').checked) {
        // Se é funcionário, verificar se também tem cargo de gerente
        const cargoId = parseInt(document.getElementById('cargo_id_cargo').value);
        if (cargoId === 2) { // ID 2 = Gerente
            tipo_usuario = 'G';
        } else {
            tipo_usuario = 'F'; // Funcionário/Vendedor
        }
    }
    ...
    try {
        ...
        switch (operacao) {
            ...
            case 'incluir':
                // criar pessoa
                respPessoa = await fetch(`${API_BASE_URL}/pessoa/api`, {
                    method: 'POST',
            ...
            case 'alterar':
               // console.log('Alterar pessoa - currentPersonId: ' + currentPersonId);
                ...
                // alterar pessoa
                respPessoa = await fetch(`${API_BASE_URL}/pessoa/api/${currentPersonId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(pessoa)
                });
            ...
            case 'excluir':
                ...
                // excluir pessoa
                const respDelPessoa = await fetch(`${API_BASE_URL}/pessoa/api/${currentPersonId}`, { method: 'DELETE' });
                if (!respDelPessoa.ok) {
...
async function carregarPessoas() {
    try {
        const response = await fetch(`${API_BASE_URL}/pessoa/api`);
...
async function popularCargosSelect() {
    ...
    try {
        ...
        const response = await fetch('/cargo/api'); 
