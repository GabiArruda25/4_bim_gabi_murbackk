...
exports.listarProdutos = async (req, res) => {
  try {
    const result = await query('SELECT id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto FROM produto ORDER BY id_produto');
...
exports.criarProduto = async (req, res) => {
  ...
  try {
    const { id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto} = req.body;

    // Validação básica
    if (!nome_produto || !preco_unitario_produto) {
    ...
    const result = await query(
      'INSERT INTO produto (id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto) VALUES ($1, $2, $3, $4, $5) RETURNING *',
      [id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto]
    );
...
exports.obterProduto = async (req, res) => {
  try {
   ...
   // console.log("estou no obter produto id="+ id);
    ...
    const result = await query(
      'SELECT id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto FROM produto WHERE id_produto = $1',
      [id]
    );
...
exports.atualizarProduto = async (req, res) => {
  try {
    ...
    const { nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto } = req.body;
    ...
    // imagem_produto
    if (imagem_produto != null && String(imagem_produto).trim() !== '') {
        updates.push(`imagem_produto = $${idx++}`);
        values.push(String(imagem_produto).trim());
    }
    ...
    values.push(id);
    const sql = `UPDATE produto SET ${updates.join(', ')} WHERE id_produto = $${idx} RETURNING id_produto, nome_produto, quantidade_estoque_produto, preco_unitario_produto, imagem_produto`;
