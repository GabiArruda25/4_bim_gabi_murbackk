...
exports.listarClientes = async (req, res) => {
  try {
    const result = await query('SELECT cli.pessoa_cpf_pessoa, p.nome_pessoa, cli.data_cadastro_cliente FROM cliente cli, pessoa p where cli.pessoa_cpf_pessoa = p.cpf_pessoa ORDER BY cli.pessoa_cpf_pessoa ');
...
exports.criarCliente = async (req, res) => {
  ...
  try {
    const { pessoa_cpf_pessoa, data_cadastro_cliente } = req.body;

    // Validação básica
    if (!pessoa_cpf_pessoa) {
      return res.status(400).json({
        error: 'O CPF da pessoa é obrigatório'
      });
    }

    const result = await query(
      'INSERT INTO cliente (pessoa_cpf_pessoa, data_cadastro_cliente) VALUES ($1, $2) RETURNING *',
      [pessoa_cpf_pessoa, data_cadastro_cliente || new Date().toISOString().split('T')[0]]
    );
...
exports.atualizarCliente = async (req, res) => {
  ...
  try {
    ...
    const updateResult = await query(
      'UPDATE cliente SET data_cadastro_cliente = $1 WHERE pessoa_cpf_pessoa = $2 RETURNING *',
      [data_cadastro_cliente, id]
    );
